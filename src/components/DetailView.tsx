...
export default function DetailView({ site, onClose }: DetailViewProps) {
    ...
    const activeData = useMemo(() => {
        if (!activeVersionId) return site;
        const version = site.versions.find(v => v.version === activeVersionId);
        return version || site; // Fallback to site if version not found
    }, [site, activeVersionId]);

    // Handle backward compatibility where history might check for 'colors' if 'topColors' missing in very old data
    const currentTopColors = (activeData as any).topColors || (activeData as any).colors || [];
    const currentTextColors = (activeData as any).textColors || [];
    const currentAllColors = (activeData as any).allColors || [];

    const displayedAllColors = showAllColors ? currentAllColors : currentAllColors.slice(0, 16);
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className="fixed inset-0 md:inset-10 md:rounded-2xl z-50 bg-[#0f0f0f] border border-[#333] overflow-y-auto md:overflow-hidden flex flex-col shadow-2xl"
            >
                <button
                    onClick={onClose}
                ...
                <div className="flex flex-col md:flex-row md:h-full">
                    ...
                    <div className="w-full md:w-1/3 p-8 md:p-12 border-b md:border-b-0 md:border-r border-[#333] flex flex-col justify-between relative overflow-hidden bg-[#111]">
                        <div>
                            ...
                            <div className="mt-8">
                                ...
                                <div className="text-white text-3xl font-serif">
                                    {format(new Date((activeData as any).date || (activeData as any).lastUpdated), 'dd MMMM yyyy')}
                                </div>
                                <div className="text-[#555] font-mono text-sm mt-1">
                                    {format(new Date((activeData as any).date || (activeData as any).lastUpdated), 'EEEE, HH:mm')}
                                </div>
                            </div>
                        </div>
                        ...
                        <motion.div
                            ...
                            className="absolute -bottom-20 -left-20 w-80 h-80 rounded-full blur-[120px] pointer-events-none"
                    ...
                    <div className="w-full md:w-2/3 p-8 md:p-12 overflow-y-auto custom-scrollbar bg-[#0f0f0f]">
                        ...
                        >
                            ...
                            {currentTopColors && currentTopColors.length > 0 ? (
                                <>
                                    <section className="mb-8">
                                        ...
                                        <div className="flex flex-wrap gap-4">
                                            {currentTopColors.slice(0, 6).map((color: Color, index: number) => (
                                    ...
                                    <section className="mb-8">
                                        ...
                                        <div className="flex flex-wrap gap-4">
                                            {(currentTextColors && currentTextColors.length > 0 ? currentTextColors : currentTopColors.filter((c: Color) => c.usage?.toLowerCase().includes('text'))).map((color: Color, index: number) => (
                                            ...
                                            {(!currentTextColors || currentTextColors.length === 0) && currentTopColors.filter((c: Color) => c.usage?.toLowerCase().includes('text')).length === 0 && (
                                    ...
                                    <section className="mb-8">
                                        <div className="flex items-center justify-between mb-6">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <button
                                                    onClick={() => handleCopyPalette(currentAllColors, 'all')}
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            {displayedAllColors.map((color: Color, index: number) => (
                                                <motion.div
                                                    key={index}
                                        ...
                                        {currentAllColors.length > 16 && (
                                            ...
                                            >
                                                {showAllColors ? 'Show Less' : `Show ${currentAllColors.length - 16} More Colors`}
                                            </button>
                                        )}
                                    </section>
                                    ...
                                    <section>
                                        ...
                                        <div className="space-y-0 relative border-l border-[#222] ml-3">
                                            {/* Latest Version Item (Explicit "Back to Latest" if looking at history) */}
                                            {activeVersionId && (
                                                <div
                                                    className="relative pl-8 py-4 group hover:bg-white/[0.02] transition-colors rounded-r-lg cursor-pointer mb-2"
                                                    onClick={() => setActiveVersionId(null)}
                                                >
                                                    <div className="absolute left-[-5px] top-6 w-2.5 h-2.5 bg-blue-500 border border-blue-400 rounded-full z-10" />
                                                    <span className="text-blue-400 text-sm font-medium">Back to Current Version</span>
                                                </div>
                                            )}

                                            {site.versions.slice(0, 3).map((version, idx) => {
                                                ...
                                                // Check if version has any colors to show
                                                const hasPalette = (version.colors && version.colors.length > 0) ||
                                                    (version.topColors && version.topColors.length > 0);

                                                return (
                                                    <div
                                                        key={idx}
                                                        className={`relative pl-8 py-6 group transition-colors rounded-r-lg ${hasPalette ? 'cursor-pointer hover:bg-white/[0.02]' : 'cursor-default opacity-50'
                                                            } ${isActive ? 'bg-white/[0.05]' : ''}`}
                                                        onClick={() => hasPalette && setActiveVersionId(isActive ? null : version.version)}
                                                    >
                                                        {/* Timeline Dot */}
                                                        <div className={`absolute left-[-5px] top-8 w-2.5 h-2.5 border rounded-full z-10 transition-all ${isActive ? 'bg-white border-white scale-125' :
                                                                hasPalette ? 'bg-[#222] border-[#444] group-hover:border-white group-hover:scale-125' :
                                                                    'bg-[#222] border-[#333]'
                                                            }`} />
                                                        ...
                                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                            <div>
                                                                <div className="flex items-baseline gap-3 mb-1">
                                                                    <span className={`font-serif text-lg ${isActive ? 'text-white' : hasPalette ? 'text-[#888] group-hover:text-white transition-colors' : 'text-[#666]'}`}>
                                                                    ...
                                                                    <span className={`text-xs px-2 py-0.5 rounded-full border ${isActive ? 'bg-white text-black border-white' : 'bg-[#1a1a1a] text-[#555] border-[#222]'}`}>{version.version}</span>
                                                                </div>
                                                                <div className="flex items-center gap-2 mt-2">
                                                                    {isActive && <span className="text-[10px] text-green-400 font-mono bg-green-400/10 px-1.5 py-0.5 rounded">Viewing</span>}
                                                                    <button
                                                                        onClick={(e) => { e.stopPropagation(); handleCopyPalette(version.colors, sectionId); }}
                                                                        className="text-[10px] flex items-center gap-1.5 px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-[#888] hover:text-white transition-colors w-fit"
                                                                    >
                                                            ...
                                                            <div className="flex items-center gap-3 opacity-60 group-hover:opacity-100 transition-opacity">
                                                                {version.colors.slice(0, 4).map((c, i) => (
